<?php
	global $config;
	if ($config['sorting']){
	if (!empty($_COOKIE['cookie_arch_topic'])) $cookie_topic = $_COOKIE['cookie_arch_topic'];
	else $cookie_topic="all_topic";
    if(!empty($_POST['user_selected_topic'])) {
	$cookie_topic=$_POST['user_selected_topic'];
	setcookie('cookie_arch_topic',$cookie_topic, FOREVER_COOKIE);
}
...
<br />
<div align="right"><?=rcms_pagination(sizeof($tpldata), $system->config['perpage'] * 3, $page + 1, '?module=forum.archive')?></div>
<table width="100%" cellpadding="2" cellspacing="1" class="forum.archiveline">
<tr>
...
<tr>
    <td class="<?=$style?>" width="100%">
        <b><?=$msg?></b>
        <a href="?module=forum.archive&amp;action=topic&amp;id=<?=$topic_id?>"><?=$topic['title']?></a>
        <?=rcms_pagination($topic['replies'], $system->config['perpage'], -1, '?module=forum.archive&amp;action=topic&amp;id=' . $topic_id)?>
    </td>
    ...
    <td class="<?=$style?>" nowrap="nowrap" align="center">        
        <?php if($topic['last_reply'] != 0 && @$topic['last_reply_id'] != 0){?>
            ...
            <?php }?>
            <a href="?module=forum.archive&amp;action=topic&amp;id=<?=$topic_id?>&amp;pid=<?=$topic['last_reply_id'] + 2?>#<?=$topic['last_reply_id'] + 2?>">&gt;&gt;</a>
        <?php } else {?>
            <?=__('No replies')?>
        <?php }?>
    </td>
...
<div align="left"><?=rcms_pagination(sizeof($tpldata), $system->config['perpage'] * 3, $page + 1, '?module=forum.archive')?></div>