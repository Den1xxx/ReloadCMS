...
class articles{
	...
	function saveArticle($cat_id, $art_id, $title, $src, $keywords, $sef_desc, $desc, $text, $mode = 'text', $comments = 'yes') {
		...
		if(empty($desc)) {							//mod 2012
			$dot = array ('.', '!', '?', '>');
			if (mb_strlen($text) > 250) {			//добавить настройку обрезки в админку
			$string = mb_substr($text, 0, 250);		
			$last_pos = 0;
			foreach ($dot as $delimiter) {			// Обрезка по последнему знаку препинания или закрытому тегу
			$pos = mb_strrpos ( $string, $delimiter);
			if (!empty($pos) && $last_pos<$pos) {
			$last_pos = $pos;
			$symbol = $delimiter;}
			}
			if (!isset ($symbol)) $desc = $string . '...';	//Ничего не найдено
			else $desc = mb_substr($string, 0, $last_pos) . '...';
			} else $desc = text;
		}
	...
	function addComment($cat_id, $art_id, $text) {
		...
		if(is_file($art_prefix . 'define')){
			...
			$newmesg['text'] = mb_substr($text, 0, 2048);
