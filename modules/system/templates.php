...
function rcms_parse_dynamik_menu($format) {
	...
	$pic_down = '<img src = \''.SKIN_PATH.'arrow_down.gif\' alt="down"/>';
	$navigation = parse_ini_file(CONFIG_PATH . 'navigation.ini', true);
  	$dyna = parse_ini_file(CONFIG_PATH . 'dynamik.ini', true);
	$result = array();
	foreach ($navigation as $link) {
	...
	$menu = ' <script type="text/javascript" src="./tools/js/navigation.js"></script> <div class="dhtml_menu"> <div class="horz_menu"> ';
	foreach ($result as $item){
		if(empty($item[2])) {
			$item[2] = '_top';
		}
		...
		$item[4] = 'ddm_article';
			...
			$item[3] = 'gallery.png';
		} elseif ($item[0] == '?module=user.list' && LOGGED_IN) {
		...
		if (isset($dyna['ico']))
			$item[3] = '<img src="./skins/icons/'.$item[3].'" alt="'.$item[3].'"/>';
		else 
			$item[3]='';
    ...
    switch($element){
        ...
        case 'navigation':
        		...
        		if (isset($dyna['use'])) {
						...
						$den1xxx = rcms_parse_menu($parameters);
						$den1xxx = str_replace('id="{id}"','',$den1xxx);
				echo $den1xxx;
            break;
        case 'meta':
